...
#import "Skeletons_CombineAnimationsTest.h"


@implementation Skeletons_CombineAnimationsTest
...
-(NSString*)initTest
{
    ...
    return @"\nDemonstrate changing skeleton animations\nwith the use of notifications.\nTap to shoot...";
}
...
//    NSLog(@"DID FINISH LOOP %d ON ANIMATION NAMED %@ ON SKELETON %@", [animation currentLoop], [animation name], skelet);
    if([[animation name] isEqualToString:@"Shoot"])
    {
        walk = true;
        [skelet playAnimationWithName:@"SoftWalk"];
    }
}
