...
#import "Skeletons_PlayAnimationFrameByFrameTest.h"


@implementation Skeletons_PlayAnimationFrameByFrameTest
...
-(NSString*)initTest
{
    ...
    [self executeTestCodeAtPosition:ccp(s.width/2, s.height/2-50)];

    glClearColor(0.5, 0.5, 0.5, 1);//white background
    
    return @"\nDemonstrate playing a skeleton animation frame by frame.\nTap on left side of the screen to go to previous frame.\nTap on right side of the screen to go to net frame.";
}
...
//        [skeleton playAnimationWithName:@"BallPlay"];
//        [skeleton playAnimationWithName:@"Bow"];
//        [skeleton playAnimationWithName:@"Death"];
//        [skeleton playAnimationWithName:@"HatWave"];
//        [skeleton playAnimationWithName:@"HolsterGun"];
//        [skeleton playAnimationWithName:@"Idle"];
//        [skeleton playAnimationWithName:@"Push"];
//        [skeleton playAnimationWithName:@"Shoot"];
//        [skeleton playAnimationWithName:@"SoftWalk"];
        [skeleton playAnimationWithName:@"StrongWalk"];
        
        GHSkeletalAnimation* animation = [skeleton animation];
...
- (void)ccTouchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
	...
	for( UITouch *touch in touches ) {
        ...
        if(location.x < s.width/2)
        {
            if(skeleton){
                GHSkeletalAnimation* animation = [skeleton animation];
                [animation goToPreviousBonePositionFrame];
            }
        }
        else{
            if(skeleton){
                GHSkeletalAnimation* animation = [skeleton animation];
                [animation goToNextBonePositionFrame];
            }
        }
	}
}
