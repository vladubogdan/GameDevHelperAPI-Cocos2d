...
#import "Skeletons_PlayAnimationByTransitioningTest.h"


@implementation Skeletons_PlayAnimationByTransitioningTest
...
-(NSString*)initTest
{
    ...
    return @"\n\n\n\nDemonstrate transitioninig to new skeleton animation.\nTap on left side of the screen to stop animation.\nTap on right side of the screen to transtion to new animation and play.\nYou should only transition animations that are related (e.g from walk to run).\nIf you are not careful this may create unrealistic movement,\nlike in this example when transitioning to sitting animation.\n";
}
...
-(void)changeAnimation{
    ...
    NSLog(@"CHANGE TO ANIMATION %@", finalAnimName);
    [skeleton transitionToAnimationWithName:finalAnimName inTime:2];
}
